	...
	)
{
	...
	buf_len -= AnscSizeOfString(buffer);
	...
	for( i = 0; i < (int)uLength; i ++)
	{
		if( i == 0)
		{
			...
			/* Reduce the buffer length after each update */
			buf_len -= rc;
		}
		else
		{
	...
	)
{
	...
	char*							pReqName     = NULL;
        errno_t         rc   = -1;
	// CID 158889 : Branch past Initialization
	int pName_length = 0;
	int pVal_len = 0;
	...
	/* check all the instances and find the first one with the type and value */
        pName_length = sizeof(pName1);
        pVal_len = sizeof(pValue);
	// CID 160776 : Logically dead code
