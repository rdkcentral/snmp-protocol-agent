...
{
    int retval = 0;
    snmp_log(LOG_INFO, "NonRoot feature is enabled, dropping root privileges for snmp_subagent process\n");
    appcaps.caps = NULL;
    ...
    */
    ...
    if (!rc)
    {
        ...
        if (0 == strcmp(buff, "true"))
        {
            ...
            fd_snmp = open("/tmp/snmp_subagent_v2_initialized", O_CREAT | O_WRONLY | O_APPEND, 644);
            if (fd_snmp < 0)
            {
                fprintf(stderr, "Failed to open /tmp/snmp_subagent_v2_initialized\n");
            }
            else
            {
                print_uptime("boot_to_snmp_subagent_v2_uptime",NULL, NULL);
    ...
    if (!rc)
    {
        ...
        if (0 == strcmp(buff, "true"))
        {
            ...
            fd_snmp = open("/tmp/snmp_subagent_v3_initialized", O_CREAT | O_WRONLY | O_APPEND, 644);
            if (fd_snmp < 0)
            {
                fprintf(stderr, "Failed to open /tmp/snmp_subagent_v3_initialized\n");
            }
            else
            {
                print_uptime("boot_to_snmp_subagent_v3_uptime",NULL, NULL);
