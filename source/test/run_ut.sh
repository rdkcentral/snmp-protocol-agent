...
log "INFO" "Running make for snmp_subagent_gtest.bin..."
...
if [ -f "./source/test/snmp_subagent_gtest.bin" ]; then
    log "INFO" "Running snmp_subagent_gtest.bin"
    ./source/test/snmp_subagent_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "snmp_subagent_gtest.bin does not exist, cannot run tests"
    exit 1
fi
