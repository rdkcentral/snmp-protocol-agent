...
BOOL Cosa_FindDestComp(char* pObjName,char** ppDestComponentName, char** ppDestPath)
{
	...
	{
		if( size > 1)
			CcspTraceInfo(("SNMP_QUERY_FOR_MULTIPLE_PROVIDERS : Multiple providers found for the '%s' property \n",pObjName));
 
		/* For backward compatability adding only the 0th index */
	...
	)
{
    ...
    if(strcmp(val->parameterName, "Device.DeviceInfo.X_RDKCENTRAL-COM_FirmwareDownloadAndFactoryReset") == 0)
    {
        v_secure_system("touch /tmp/snmpFWFR");
    }
